...
A script that scrapes [[Wayback Machine]] pages given a CSV of URLs



1. Reads the existing CSV to get the list of articles to be scraped.
2. Fetches the HTML content and parses its metadata and content using [[beautifulsoup4]]  and a number of host-specific rules.
3. Saves each scraped article to the relevant host’s directory in the output path.
4. Saves metadata for all articles into `raw.csv`.
